...
def main():
    test_sizes = [8000, 10000, 12000, 14000, 16000, 18000, 20000]
    seq_times = []
    ...
    plt.plot(n_smooth, t_theoretical, 'r--', linewidth=3,
             label=f'Теоретическая O(n³/P)')

    plt.xlabel('Количество чисел', fontsize=13)
    plt.ylabel('Время выполнения (секунды)', fontsize=13)
    plt.title('Проверка гипотезы Гольдбаха с многопоточностью', fontsize=14, pad=15)
