...
import numpy as np
from интерполяция import interpolation_search_primes
...
def find_goldbach_numbers(max_N):
    ...
    for i, a in enumerate(primes):
        ...
        if j_max >= i:
            for j in range(i, min(j_max + 1, len(primes))):
                b = primes[j]
                s = a + b
...
N_values = [10000, 30000, 50000, 150000, 200000, 250000]
times = []
for N in N_values:
...
plt.plot(N_arr, T_real, 'o-', color='green', label='Реальное время выполнения', linewidth=2)
plt.plot(N_arr, T_theory, '--', color='red', linewidth=2, label=fr'Теоретическое O(n²/(ln(n))²) k={c:.2e}')
plt.xlabel('Количество чисел')
plt.ylabel('Время выполнения (секунды)')
plt.title('Проверка гипотезы Гольдбаха с интерполяционным поиском')
plt.legend()
